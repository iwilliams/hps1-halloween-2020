...
export(NodePath) var target_position_path: NodePath
onready var target_position = get_node(target_position_path)
#
#func drop() -> void:
#	_holder = null
#
...
#	var pos_target := _holder.get_global_transform()
    var pos_target = target_position.get_global_transform()
    ...
    state.linear_velocity = (pos_next.origin - pos_cur.origin) / delta * .5
